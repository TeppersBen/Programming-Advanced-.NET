	...
	public class Moq3_MethodsTest : Koan
	{
		...
		public void UsingWhatYouHaveLearned_CreateACompletelyFunctionalIVolumeMock()
		{
		    ...
		    mock.Setup(m => m.Louder(It.Is<int>(p => p < 0)))
		        .Throws(new ArgumentOutOfRangeException("Louder did not throw an exception on a negative input."));
		    mock.Setup(m => m.Quieter(It.Is<int>(p => p < 0)))
		        .Throws(new ArgumentOutOfRangeException("Quieter did not throw an exception on a negative input."));
